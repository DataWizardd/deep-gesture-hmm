...
class Metrics:
    def __init__(self, fps_alpha=0.2, time_alpha=0.2, spot_window=20):
        ...
        self.spot_hist = defaultdict(
            lambda: deque(maxlen=spot_window)
        )  # 라벨별 최근 N개
        self.queue_depth = 0
    ...
    def update_times(self, end2end_ms=None, enc_ms=None, hmm_ms=None):
...
def main():
    ...
    try:
        while cap.isOpened():
            ...
            e2e_txt = f"End2End: {metrics.end2end_ms_ema:5.1f} ms"
            enc_txt = f"Enc: {metrics.enc_ms_ema:5.1f} ms"
            hmm_txt = f"HMM: {metrics.hmm_ms_ema:5.1f} ms"
            q_txt = f"Q:{metrics.queue_depth}"

            spot_txt = ""
            if last_predicted_gesture:
                avg_ms = metrics.avg_spot_ms(last_predicted_gesture)
                if avg_ms is not None:
                    spot_txt = f" | Spotting Avg({COMMAND_MAP.get(last_predicted_gesture,'')}): {avg_ms:5.1f} ms"

            metrics_line = (
                f"{fps_txt} | {e2e_txt} | {enc_txt} | {hmm_txt} | {q_txt}{spot_txt}"
            )
